...
namespace CSharpSynth.Sequencer {
    public class MidiSequencer {
        ...
        private StreamSynthesizer synth; // changed to public
        ...
        public virtual void BeforeCombineTracks(MidiFile midi) { }
        public virtual bool LoadMidi(MidiFile midi, bool UnloadUnusedInstruments) // edited to make overrideable
        {
            if (playing == true)
                return false;
            _MidiFile = midi;
            if (_MidiFile.SequencerReady == false) {
                try {
                    ...
                    BeforeCombineTracks(midi);
                    _MidiFile.CombineTracks(); // this makes dealing with different things a pain in the ass. // but it will make everything poop out if we don't call it
            ...
            if (UnloadUnusedInstruments == true) {
                if (synth.SoundBank == null) {//If there is no bank warn the developer =)
        ...
        public bool LoadMidi(string file, bool UnloadUnusedInstruments)  // don't need to edit because it calls the other LoadMidi which can be overriden
        {
			if(file == null || file.Length == 0) {
				Debug.LogWarning("file.Length is 0 or null. file: " + file);
			}
            if (playing == true)
                return false;
            MidiFile mf = null;
        ...
        public virtual void ProcessMidiEvent(MidiEvent midiEvent) // edited to make overrideable
        {
        ...
        private void SetTime(TimeSpan time) {
            ...
            } else if (_stime < sampleTime) {//we have to restart the midi to make sure we get the right temp, instrument, etc
                synth.Stop();
                sampleTime = 0;
                Array.Clear(currentPrograms, 0, currentPrograms.Length);
                ResetControllers();
        ...
        private void SilentProcess(int amount) { // agggghhhhhhh why did the person who made this not know how to define a local variable???
