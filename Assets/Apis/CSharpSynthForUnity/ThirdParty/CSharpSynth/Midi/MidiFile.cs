    ...
    public class MidiFile {
        ...
        public MidiFile(string filename) {
            ...
            try {
                ...
                throw new Exception("Midi Failed to Load! filename: '" + filename +"'", ex);
            } finally {
                if (midiStream != null)
                    midiStream.Dispose();
            }
        ...
        public void CombineTracks() // Called from LoadMidi
        {
            if (tracks.Length < 2)
                return;
            ...
            for (int x = 0; x < tracks.Length; x++) {
                ...
                for (int x2 = 0; x2 < tracks[x].MidiEvents.Length; x2++) {
                    CurrentDeltaTime_ += TrackSeq[x2].deltaTime; // edited to make more clear (added +=)
                    TrackSeq[x2].deltaTime = CurrentDeltaTime_; // I guess this makes the delta time the actual time it plays. // but remember, this is not only for the TrackSeq stuff because the instances should have been updated to
        ...
        private void loadStream(Stream stream) {
            ...
            for (int x = 0; x < trackCount; x++) {
                ...
                while (index < tmp.Length) {
                    ...
                    prevByte = statusByte;
                    prevChan = CHANNEL;
                    switch (statusByte) // just thought this might help for i++ and ++i: https://stackoverflow.com/questions/6175316/the-difference-between-var-and-var // pain in the ass tho plz don't do that
                    {
                        case 0x80: {
